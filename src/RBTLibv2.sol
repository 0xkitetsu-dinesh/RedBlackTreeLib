...
library RedBlackTreeLib {
    ...
    uint24 private constant EMPTY = 0;
    ...
    function getKey(Tree storage self,uint value) internal view returns (uint24) {
        require(value != EMPTY,"value != EMPTY");
        uint24 probe = self.root;
        while(probe != EMPTY){
    ...
    function rotateLeft(Tree storage self, uint24 key) private {
        uint24 cursor = self.nodes[key].right;
        uint24 keyParent = self.nodes[key].parent;
        uint24 cursorLeft = self.nodes[cursor].left;
    ...
    function rotateRight(Tree storage self, uint24 key) private {
        uint24 cursor = self.nodes[key].left;
        uint24 keyParent = self.nodes[key].parent;
        uint24 cursorRight = self.nodes[cursor].right;
    ...
    function insertFixup(Tree storage self, uint24 key) private {
        uint24 cursor;
    ...
    function insert(Tree storage self, uint160 value) internal {
        ...
        uint24 newNodeIdx = ++self.totalNodes;
    ...
    function replaceParent(Tree storage self, uint24 a, uint24 b) private {
        uint24 bParent = self.nodes[b].parent;
    ...
    function removeFixup(Tree storage self, uint24 key) private {
    ...
    function remove(Tree storage self, uint value) internal {
        ...
        uint24 yParent = self.nodes[cursor].parent;
