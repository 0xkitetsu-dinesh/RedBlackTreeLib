...
contract RedBlackTestv2 is Test {
    ...
    function equal(BokkyPooBahsRedBlackTreeLibrary.Tree storage bpb_tree,RedBlackTreeLib.Tree storage rb_tree) internal returns(bool){
        ...
        if (bpb_tree.root != EMPTY){
            ...
            for(uint i = 1;i<= rb_tree.totalNodes;i++){
                ...
                require(bpb_val == rb_val,string.concat("parent mismatch ",uint2str(bpb_val)," : ",uint2str(rb_val)));
    ...
    function insertRBtree(RedBlackTreeLib.Tree storage rb_tree,uint160 val,bool debug) internal returns(uint){
        uint tree_size = rb_tree.size();
        uint GAS_BEFORE = gasleft();
        rb_tree.insert(val);
        uint GAS_USED = GAS_BEFORE - gasleft();
        if (debug) console.log(string.concat("Tree size - ",uint2str(tree_size)," | Gas used after inserting -> ",uint2str(val)),GAS_USED);
        return GAS_USED;
    }
    function insertBPBtree(BokkyPooBahsRedBlackTreeLibrary.Tree storage bpb_tree,uint160 val,bool debug) internal returns(uint){
        uint GAS_BEFORE = gasleft();
        bpb_tree.insert(val);
        uint GAS_USED = GAS_BEFORE - gasleft();
    ...
    function removeRBtree(RedBlackTreeLib.Tree storage rb_tree,uint val,bool debug) internal returns(uint){
        ...
        rb_tree.remove(val);
        uint GAS_USED = GAS_BEFORE - gasleft();
        if (debug) console.log(string.concat("Tree size - ",uint2str(tree_size)," | Gas used after removing -> ",uint2str(val)),GAS_USED);
        return GAS_USED;
    }
    ...
    function setUp() public {
        ...
        uint160 temp = 1 ;
        for (uint i;i<1000;i++){
            temp = uint160(uint(keccak256(abi.encode(temp))));
    ...
    function test() public {
        ...
        for (uint i ;i<input_delete.length;i++){
            ...
            uint gas_used_RB = removeRBtree(RBtree,input_delete[i],false);
            uint gas_used_BPB = removeBPBtree(BPBtree,input_delete[i],false);
